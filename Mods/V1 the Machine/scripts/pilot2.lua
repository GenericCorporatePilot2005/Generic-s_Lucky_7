...
-- add assets - notice how the name is identical to pilot.Id
    ...
    modApi:appendAsset("img/combat/icons/Nico_V2_icon.png", path.."img/combat/icons/Nico_V2_icon.png")
	Location["combat/icons/Nico_V2_icon.png"] = Point(-18,7)
...
function this:init(mod)
	...
	-- art, icons, animations
		...
		function Nico_V2skill:GetSkillEffect(p1, p2)
			...
			if Board:IsPawnSpace(p2) and Board:GetPawn(p2):IsArmor() then -- and Board:GetPawn(p2):GetDefaultFaction() ~= FACTION_BOTS and Board:GetPawnTeam(p2) == TEAM_ENEMY then
				damage.sImageMark = "combat/icons/Nico_V2_icon.png"
				ret:AddScript([[
				...
				ret:AddScript([[local cast = { main = ]]..Board:GetPawn(p1):GetId()..[[ }
					modapiext.dialog:triggerRuledDialog("Mission_Nico_V2GBreak", cast)
				]])
				...
				if Board:GetPawn(p2):GetType() ~= "tosx_IceHornet" then ret:AddScript("Board:GetPawn("..web_id.."):SetSpace(Point(-1,-1))") end--Move the pawn to Point(-1,-1) to delete webbing
					...
					damage0.bHide = true
					damage0.fDelay = 0.00017--force a one frame delay on the board
