...
local icon = sdlext.surface("img/combat/icons/icon_massive.png")
...
local newicon
local iconSprite = "img/effects/icon_massive_orbReload.png"
local pawnTypeShown = false


local function IsReloader(pawn)
	local pawnType
	local Nico_Orbital_Pilot
	
	if pawn then
		pawnType = pawn:GetType()
		Nico_Orbital_Pilot = pawn:IsAbility("Nico_Orbital_Skill")
	...
	return Nico_Orbital_Pilot or pawnData
end
...
local function UiRootCreatedHook(screen, uiRoot)
	...
	missionSmallWidget = Ui()
		:widthpx(21):heightpx(21)
		:addTo(uiRoot)
	local mask = UiCover({size = {21, 21}})
...
local function finalizeInit(self)	
	...
	function GetStatusTooltip(id)
		if	id == "massive"		and
			IsReloader(Pawn)	then
			
			return {"Satellite Reloader", "This Unit is Massive, when this unit triggers deadly damage, reloads the Recon Satellite's Weapon."}
		end
...
local function onModsInitialized()
	local isHighestVersion = true
		and ReloaderIcon.initialized ~= true
		and ReloaderIcon.version == VERSION

	if isHighestVersion then
		ReloaderIcon:finalizeInit()
		ReloaderIcon.initialized = true
	end
end
...
local isNewerVersion = false
	or ReloaderIcon == nil
	or VERSION > ReloaderIcon.version

if isNewerVersion then
	ReloaderIcon = ReloaderIcon or {}
	ReloaderIcon.version = VERSION
	ReloaderIcon.finalizeInit = finalizeInit
