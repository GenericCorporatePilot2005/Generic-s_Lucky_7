...
function this:init(mod)
	...
	function Nico_Tauntskill:GetSkillEffect(p1, p2)
		...
		if gotTaunted then
			...
			if not (Board:GetPawn(p1):IsShield() or Board:GetPawn(p1):IsFrozen() or Board:GetCustomTile(p1) == "tosx_rocks_0.png") then
				shield.iShield = 1
			else
				shield.bHide = true
			end
			shield.iFire = EFFECT_REMOVE
			shield.iAcid = EFFECT_REMOVE
			ret:AddBounce(p1,-1)
