...
function taunt.addTauntEffectEnemy(effect, id, point, dmg, push, failFlag)
	local ret = false
	
	push = push or DIR_NONE
	dmg = dmg or 0
	...
	local loc = pawn:GetSpace()
	local dir = GetDirection(point - loc)
	if push == 0 then push = DIR_NONE
	elseif push == 1 then push = dir
	elseif push == 2 then push = GetDirection(loc - point)
	elseif push == 3 then push = (dir+1)%4
	elseif push == 4 then push = (dir-1)%4
	elseif push == 5 then push = DIR_FLIP
	else push = DIR_NONE end

	effect:AddScript([[
		taunt.enemy(]]..id..[[,]]..point:GetString()..[[)
	]])
	local damage = SpaceDamage(loc,dmg,push)
...
function taunt.addTauntEffectSpace(effect, space, point, dmg, push, failFlag)
	dmg = dmg or 0
	failFlag = failFlag or false
	push = push or 0
	if Board == nil --[[or IsTipImage()]] then return end
	...
	if pawn == nil or not isEnemyPawn(pawn) then
		...
		if push == 0 then push = DIR_NONE
		elseif push == 1 then push = dir
		elseif push == 2 then push = GetDirection(space - point)
		elseif push == 3 then push = (dir+1)%4
		elseif push == 4 then push = (dir-1)%4
		elseif push == 5 then push = DIR_FLIP
		else push = DIR_NONE end

		local damage = SpaceDamage(space,dmg,push)
		damage.sImageMark = "combat/icons/tauntFailIcon_"..tostring(dir)..".png"
		if failFlag then
			damage.iDamage = 0
		elseif dmg > 0 then
	...
	local id = pawn:GetId()
	return taunt.addTauntEffectEnemy(effect,id,point,dmg,push,failFlag)
end
